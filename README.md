...
*   **商品情報編集**: 商品の詳細情報（商品名、URL、カテゴリ、サブカテゴリ、画像）を編集可能。**RLSポリシーにより、自分が作成した商品のみ編集できるセキュリティ機能を実装**。
...
*   **バックエンド**: Supabase (PostgreSQL Database, Supabase Auth, Supabase Storage, **Row Level Security**)
*   **ユーティリティ**: **flutter_dotenv** (環境変数管理), **image** (画像処理), **flutter_image_compress** (WebP/JPEG画像圧縮、プラットフォーム別最適化)
*   **バージョン管理**: Git, GitHub
