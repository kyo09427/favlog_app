...
- **Logtoログイン**: Logto（OIDCプロバイダー）経由のOAuth認証
- **メール/パスワード認証**: 既存ユーザー向けに継続対応
...
| **Supabase Auth** | ユーザー認証・セッション管理（Custom Auth Provider経由） |
...
| **Logto** | 認証基盤（OIDCプロバイダー）・複数アプリの認証を一元管理 |
...
Discord認証をLogto OIDCプロバイダー経由の認証に移行。

#### 認証基盤の変更
- **Logto認証への移行**: SupabaseのCustom Auth Provider機能を使い、LogtoをOIDCプロバイダーとして設定。複数アプリの認証を一元管理できる基盤を構築
- **Discordボタン → Logtoボタン**: AuthScreenのログインボタンをLogto向けに変更
- **Discord Guild検証を一時無効化**: Logto経由ではDiscordのprovider tokenをSupabaseが受け取れないため、Guild検証ロジックを一旦無効化（将来的にLogtoのカスタムクレームで対応予定）

#### 不具合対処（メモ）
- Supabase Custom Auth ProviderのScopesに `profile` を含めると、LogtoのIDトークンに `updated_at`（ミリ秒）が含まれSupabase GoTrueがクラッシュする。Scopesは `openid, email` のみを使用すること

---

### v2.8.0（2026-04-12）

アップデート機能・Discord ログイン機能のコード品質・堅牢性を全面改善したリファクタリングリリース。
...
#### バグ修正・セキュリティ強化（Discord ログイン）
- **Edge Function エラー時の誤強制ログアウト修正**: Discord API 一時障害（502）・内部エラー（500）等のステータスを「ギルド未参加（403）」と同一視して正規ユーザーをログアウトさせていた問題を修正。403 のみを未参加判定とし、それ以外のエラーは例外として呼び出し側でログのみ記録する方式に変更
- **`response.data` の unsafe キャスト修正**: `response.data as String` が型不一致時にクラッシュしていた問題を修正。`String` / `Map` / `null` それぞれに対応した安全な型変換処理に変更
- **ギルドキック後のアクセス継続問題を修正**: Discord providerToken が null の場合（トークン期限切れ等）にギルド検証をスキップしていたため、ギルドキック後もセッション期間中アクセス可能だった問題を修正。providerToken が null の Discord ユーザーは強制ログアウトし、再認証を促すよう変更
- **Discord ボタン二重タップ防止**: `signInWithOAuth()` はブラウザを開いた時点で完了し、OAuth フロー中もボタンが有効に戻る問題を修正。`WidgetsBindingObserver` でアプリ復帰を検知し、ブラウザが開いている間はローディング状態を維持するよう変更

#### コード品質改善（Discord ログイン）
- **Edge Function の環境変数チェック追加**: `DISCORD_TARGET_GUILD_ID` が未設定の場合、全ユーザーがサイレントに拒否される問題を修正。起動時に明示的な 500 エラーを返すよう変更
- **未使用定数の削除**: `constants.dart` の `discordTargetGuildId` は Edge Function 側で管理されており Dart コードから参照されていないため削除
- **Discord guilds API に `limit=200` を明示**: デフォルト値に依存せず Discord API の最大取得件数を明示的に指定
