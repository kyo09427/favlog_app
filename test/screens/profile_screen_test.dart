...
void main() {
  ...
  group('ProfileScreen', () {
    ...
    setUp(() {
      ...
      when(
        () => mockSupabaseStorageClient.from(any()),
      ...
      when(
        () => mockReviewRepository.getReviewsByUserId(any()),
      ).thenAnswer((_) async => []);
      when(
        () => mockLikeRepository.getAllUserLikedReviewIds(any()),
      ).thenAnswer((_) async => []);
    });
    ...
    testWidgets('displays loading indicator initially', (tester) async {
      when(() => mockProfileRepository.fetchProfile(any())).thenAnswer((
        _,
      ) async {
    ...
    testWidgets('displays existing profile data', (tester) async {
      ...
      when(
        () => mockProfileRepository.fetchProfile(any()),
    ...
    testWidgets('allows picking and uploading avatar via edit dialog', (
      tester,
    ) async {
      final initialProfile = Profile(id: 'test_user_id', username: 'TestUser');
      ...
      when(
        () => mockImageCompressor.compressImage(
          any(),
          maxWidth: any(named: 'maxWidth'),
          maxHeight: any(named: 'maxHeight'),
      ...
      verify(
        () => mockStorageFileApi.uploadBinary(
          ...
          fileOptions: any(
            named: 'fileOptions',
            that: isA<FileOptions>().having(
              (fo) => fo.contentType,
              'contentType',
              anyOf('image/webp', 'image/jpeg'),
            ),
          ),
        ),
      ).called(1);
      ...
      verify(
        () => mockStorageFileApi.getPublicUrl(
          any(that: contains(RegExp(r'\.(webp|jpg)'))),
        ),
      ).called(1);
