...
void main() {
  ...
  Widget createTestWidget(Widget child, HomeScreenState initialState) {
  ...
  testWidgets(
    'shows loading shimmer when state is loading and products are empty',
    (tester) async {
      // Arrange
      final initialState = HomeScreenState(isLoading: true, products: []);
      await tester.pumpWidget(
        createTestWidget(const HomeScreen(), initialState),
      );
  ...
  testWidgets(
    'shows empty message when there are no products and not loading',
    (tester) async {
      // Arrange
  ...
  testWidgets(
    'shows product list when products are available and not loading',
    (tester) async {
      // Arrange
      ...
      final productWithStats = ProductWithReviewAndStats(
        product: product,
        latestReview: review,
        stats: stats,
      );
      ...
      when(
        () => mockCommentRepository.getCommentsByReviewId(any()),
      ).thenAnswer((_) async => []);
      when(
        () => mockLikeRepository.getLikeCounts(any()),
      ).thenAnswer((_) async => {'r1': 0});
      when(
        () => mockLikeRepository.hasUserLiked(any(), any()),
      ...
      expect(
        find.byWidgetPredicate(
          (widget) => widget is ListView || widget is GridView,
        ),
        findsOneWidget,
      );
    },
